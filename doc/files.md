...
The data directory is the default location where the 10Seconds Core files are stored.

1. The default data directory paths for supported platforms are:

Platform | Data directory path
---------|--------------------
Linux    | `$HOME/.10seconds/`
macOS    | `$HOME/Library/Application Support/10Seconds/`
Windows  | `%APPDATA%\10Seconds\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `10seconds.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual 10Seconds blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
`./`               | `10seconds.conf`        | User-defined [configuration settings](bitcoin-conf.md) for `10secondsd` or `10seconds-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `10secondsd.pid`        | Stores the process ID (PID) of `10secondsd` or `10seconds-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `10secondsd` or `10seconds-qt`; can be specified by `-debuglogfile` option
...
`10seconds-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by 10Seconds Core:
