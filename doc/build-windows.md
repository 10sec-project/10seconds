...
Below are some notes on how to build 10Seconds Core for Windows.

The options known to work for building 10Seconds Core on Windows are:
...
Note that for WSL the 10Seconds Core source path MUST be somewhere in the default mount file system, for
example /usr/src/10seconds, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\10seconds`, for example:

    make install DESTDIR=/mnt/c/workspace/10seconds
