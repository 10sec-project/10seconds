...
def download_binary(tag, args) -> int:
    ...
    if tag < "v23" and platform in ["x86_64-apple-darwin", "arm64-apple-darwin"]:
        platform = "osx64"
    tarball = '10seconds-{tag}-{platform}.tar.gz'.format(
    ...
    ret = subprocess.run(['tar', '-zxf', tarball, '-C', tag,
                          '--strip-components=1',
                          '10seconds-{tag}'.format(tag=tag[1:])]).returncode
    if ret:
        return ret
...
def build_release(tag, args) -> int:
    githubUrl = "https://github.com/10sec-project/10seconds"
    if args.remove_dir:
        if Path(tag).is_dir():
            shutil.rmtree(tag)
    ...
    with pushd(tag):
        ...
        files = ['10secondsd', '10seconds-cli', '10seconds-tx']
        for f in files:
