...
with the 10secondsd(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a 10secondsd.  `P2PInterface`
contains the higher level logic for processing P2P payloads and connecting to
the 10Seconds Core node application logic. For custom behaviour, subclass the
...
Test objects for interacting with a 10secondsd node over the p2p interface.
